...
void MtMove(void)
{
	...
	for (int i = 0; i < ROIStorageCount; i++)
	{
		...
		case 0://在这里Z值为负值，不知道原因，需要进一步勘测
			...
			sprintf(mybuffy, "%f", ROICameraSP_MachineCoord_Storage[i].Y * 1000 -60);//在这里调整Z的值的时候，一定要在MtCheck里面也对于存下来的点的值进行调整
		...
		case 1://在这里Z值为负值，不知道原因，需要进一步勘测
			...
			sprintf(mybuffy, "%f", ROICameraSP_MachineCoord_Proj_Storage[i].Y * 1000 - 60);
			sprintf(mybuffz, "%f", ROICameraSP_MachineCoord_Proj_Storage[i].Z * 1000 + 190/* - ROICameraSP_MachineCoord_Proj_Storage[i].Y * 1000 * tan(dev_theta)*/);
			break;
