...
{
	if (PlaneDepthCount < 1000)//小于1000的时候在画线
	{
		glPushMatrix();
		glPointSize(2.0f);
		glBegin(GL_LINES);
	...
	else if (PlaneDepthCount >= 1000)//大于1000的时候画的是面
	{
		glPushMatrix();
		glPointSize(5.0f);
		glBegin(GL_POINTS);
...
void RenderScene()
{
	...
	if (ROICameraSP_MechCoord != nullptr)
	{
		...
		glPrintf("(%.3f, %.3f, %.3f)", ROICameraSP_MechCoord[0].X * 1000 - 81, ROICameraSP_MechCoord[0].Y * 1000 - 69, ROICameraSP_MechCoord[0].Z * 1000 + 170);
	}
	glPopMatrix();
