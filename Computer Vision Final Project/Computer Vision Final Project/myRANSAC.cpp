...
int myRANSAC::runRansac(cv::Mat image) {
	extractPoints(image);
...
void myRANSAC::extractPoints(cv::Mat& img) {
	allPoints.clear();
...
//Because the input data is from camera, so we need to check it first
void myRANSAC::createLine() {
	//cout << allPointsLength << endl;
	if (allPointsLength != 0) {
		//count = allPointsLength*0.01;
		while (count < 2000) {
			int inLiers = 0;
			...
			//Choose 1/3 of the points to be test points
