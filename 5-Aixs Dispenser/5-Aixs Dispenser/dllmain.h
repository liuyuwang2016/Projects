...
HANDLE hCom, handle;//句柄的作用是在C语言环境下代替C++的对象指针
OVERLAPPED oCOM;//一个结构体，保存了一些一步操作的相关上下文信息
DCB dcb;//设备控制块
...
DWORD threadid = 0;//DWORD就是unsigned long无符号长整型
long flowNumber;
extern char cmd[100];
char data[40][30];
extern char SendCmdFlag;//extern声明一个值，但是不能直接用
...
extern DWORD WINAPI MotionThreadProc(LPVOID lpParameter);//LPVOID这个是一个宏定义, 表示的是指向空类型的指针
char cmdbuf[510][80];
...
long baud = 115200;//baud是波特。调制速率，指的是有效数据信号调制载波的速率，也就是单位时间内载波调制状态变化的次数
...
long EpcioCheckSum(char *s)//这个是什么意思
{
	int i;
	long cs;
	int len;
...
int EpcioLen()
{
	...
	return  comstat.cbInQue;//cbInQue The number of bytes received by the serial provider but not yet read by a ReadFile operation
}
...
void EpcioCmdFormat(char *s, char *r)
{
	...
	cs = Calc_CRC(temp);
	sprintf(r, "    %d:%s\n", cs, temp);//%d 十进制打印， %s字符打印，这里的控制形式就是MtCmd里面的控制形式
}
