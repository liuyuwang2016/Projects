...
public:
	...
	{
		 ConvexTemplate* mConvexAPtr;
		 ConvexTemplate* mConvexBPtr;

		bool mEnableMargin;

		MinkowskiDiff( ConvexTemplate* a,
			 ConvexTemplate* b)
			:mConvexAPtr(a),
			mConvexBPtr(b)
		{
		...
		SupportPoint generateSupport(Vector3 directionWorld, int* uniqueIdCounter) 
		{
	...
	struct Edge
	{
		...
		inline Edge( SupportPoint &a,  SupportPoint &b)
		{
			points[0] = a;
			points[1] = b;
		}
	...
	struct Triangle
	{
		...
		inline Triangle( SupportPoint &a,  SupportPoint &b,  SupportPoint &c)
		{
			points[0] = a;
			points[1] = b;
			points[2] = c;
	...
	static bool extrapolateContactInformation( Triangle* triangle, ContactBasicData* contactData)
	{

		// project the origin onto the triangle
		 float distanceFromOrigin = dot(triangle->n, triangle->points[0].v);
		...
		// penetration depth
		 float penetrationDepth = distanceFromOrigin;
	...
	static bool gjk_simtest(Vector3& v, Vector3& ao)
	{
		return (v).dot(ao) > 0;
	}
	...
	template <typename ConvexTemplate>
	static bool GJKIntersectionTest(Simplex* inout_simplex, int* uniqueIdCounter,  MinkowskiDiff<ConvexTemplate>&  MinkowskiShape)
	{
		...
		**************************/
		 unsigned _EXIT_ITERATION_LIMIT = 30;
	...
	template <typename ConvexTemplate>
	static bool EPAManifoldExtraction( Simplex* in_initialSimplex, int* uniqueIdCounter, MinkowskiDiff<ConvexTemplate>  MinkowskiShape, ContactBasicData* inout_contactData)
	{
		...
		// it will continue to do this until if fails to push the polytope more than _EXIT_GROWTH_THRESHOLD
		 float _EXIT_GROWTH_THRESHOLD = 0.002f;
		 unsigned _EXIT_ITERATION_LIMIT = 30;
		...
		while (true)
		{
			...
			{
				 float dst = dot(it->n, it->points[0].v);
			...
			// get the next support point in front of the current triangle, away from the origin
			 SupportPoint entry_new_support = MinkowskiShape.generateSupport(closestTriangle_it->n, uniqueIdCounter);
			...
			// on the hull from the origin
			 float newDst = dot(closestTriangle_it->n, entry_new_support.v);
			 float growth = newDst - closestTriangle_dst;
	...
	template <typename ConvexTemplate>
	static bool CollisionSolveGJK_EPA( ConvexTemplate* a,
		 ConvexTemplate* b,
