...
#pragma endregion rotation
	...
	else if (key == GLUT_KEY_F5 )//直接控制X轴的增加移动
	{
		md->x += 1;
		Mt_x = md->x;
		Mt_XMove(Mt_x);
	...
	else if (key == GLUT_KEY_F6)//直接控制X轴的增加移动
	{
		md->x -= 1;
		Mt_x = md->x;
		Mt_XMove(Mt_x);
...
void MoveCubicInY_Axis()//换了模型之后有影响
{
	...
	if (ROIStorageCount > 0 && CUBIC_MOVE)
	{
		...
		case 0:
			dy = ROICameraSP_MachineCoord_Storage[mtmove_step].Y -0.069;
			break;
		case 1:
			dy = ROICameraSP_MachineCoord_Proj_Storage[mtmove_step].Y -0.069;
			break;
		}
...
void RenderScene()
{
	...
	if (ROICameraSP_MechCoord != nullptr)
	{
		...
		glPrintf("(%.3f, %.3f, %.3f)", ROICameraSP_MechCoord[0].X * 1000 - 81, ROICameraSP_MechCoord[0].Y * 1000 - 59, ROICameraSP_MechCoord[0].Z * 1000 + 190);
	}
	glPopMatrix();
