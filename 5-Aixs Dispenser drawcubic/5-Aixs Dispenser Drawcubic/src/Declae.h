...
void MtMove(void)
{
	...
	for (int i = 0; i < ROIStorageCount; i++)
	{
		...
		case 0://在这里Z值为负值，不知道原因，需要进一步勘测
			...
			sprintf(mybuffy, "%f", ROICameraSP_MachineCoord_Storage[i].Y * 1000 -59);//在这里调整Z的值的时候，一定要在MtCheck里面也对于存下来的点的值进行调整
			sprintf(mybuffz, "%f", ROICameraSP_MachineCoord_Storage[i].Z * 1000 +190/*- (ROICameraSP_MachineCoord_Storage[i].Y * 1000 + 40)* tan(dev_theta)*/);
			break;
		case 1://在这里Z值为负值，不知道原因，需要进一步勘测
			sprintf(mybuffx, "%f", ROICameraSP_MachineCoord_Proj_Storage[i].X * 1000 - 81);
			sprintf(mybuffy, "%f", ROICameraSP_MachineCoord_Proj_Storage[i].Y * 1000 - 59);
			sprintf(mybuffz, "%f", ROICameraSP_MachineCoord_Proj_Storage[i].Z * 1000 + 220 /*- ROICameraSP_MachineCoord_Proj_Storage[i].Y * 1000 * tan(dev_theta)*/);
			break;
...
void MtCheck(void)
{
	for (mtmove_step = 0; mtmove_step < ROIStorageCount; mtmove_step++)
	{
		...
		case 0:
			...
			value = ROICameraSP_MachineCoord_Storage[mtmove_step].Z * 1000  + 190/*- ROICameraSP_MachineCoord_Storage[mtmove_step].Y * 1000 * tan(dev_theta)*/;
			break;
		case 1:
...
void ARFunc_FindProj()//换了模型之后有影响
{
	...
	//求解OBJ檔頂面之平面方程式
	ARFuncNormal1[0] = CubeNorm[0].X - CubeNorm[1].X;
	ARFuncNormal1[1] = CubeNorm[0].Y - CubeNorm[1].Y;
	ARFuncNormal1[2] = CubeNorm[0].Z - CubeNorm[1].Z;
	ARFuncNormal1[3] = -(ARFuncNormal1[0] * CubeTop[0].X + ARFuncNormal1[1] * CubeTop[0].Y + ARFuncNormal1[2] * CubeTop[0].Z);
