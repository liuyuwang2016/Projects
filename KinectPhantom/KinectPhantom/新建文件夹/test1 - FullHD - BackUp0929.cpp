...
//static void   ARTKCleanup(void);
//static void   ARTKDraw(void);
//
...
void SpecialKeys(int key, int x, int y)
{
	...
	if (key == GLUT_KEY_F1)
	{
		...
		{
			IS_KEY_F1_UP = FALSE;
			cout << endl << "IS_KEY_F1_UP == FALSE" << endl;

		}
		else
		{
			IS_KEY_F1_UP = TRUE;
			cout << endl << "IS_KEY_F1_UP == TRUE" << endl;
...
void KineticUpdate()
{
	...
	//	//}
	//
	//	//dataPtr = pBufferColor_RGB;
	//	dataPtr = FrameFormateCVtoARTK(pBufferColor);
	//
...
//	argConvGlpara(patt_trans, gl_para);
//
//	cout << gl_para[0] << " " << gl_para[1] << " " << gl_para[2] << " " << gl_para[3] << " " << endl;
//
//	glMatrixMode(GL_MODELVIEW);
//	glLoadIdentity();
//	glLoadMatrixd(gl_para);
