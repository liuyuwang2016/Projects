...
int DRAWING_TYPE = 2;                      //Flag for drawing type: 0 = none, 1 = mesh, 2 = point cloud
void DrawPointCloud(void);                 //Drawing Type
void DrawMeshes(void);


//Draw Cubic
const float CubicUnitW = 0.1f * 3;
const float CubicUnitH = 0.05f * 3;
const float CubicUnitT = 0.05f;
Point3f CubicPosi;
Point3f CubicRota;
...
void RenderScene()
{
	//CameraSpacePoint* test = new CameraSpacePoint[1];
	//test[0].X = 0;
	//test[0].Y = 0;
	//test[0].Z = CubicUnitT;
	//
	//GLfloat M[6] = { CubicRota.x, CubicRota.y, 0, CubicPosi.x, CubicPosi.y, CubicPosi.z };
	////CubicPosi.x, CubicPosi.y, CubicPosi.z
	//PathGenCoordCubicToWorld(test, 1, M, test);

	//GLfloat MM[6] = { -CubicRota.x, -CubicRota.y, 0, -CubicPosi.x, -CubicPosi.y, -CubicPosi.z };

	//PathGenCoordWorldToCubic(test, 1, MM, test);
	...
	if (ROIDepthCount != 0)
	{
		...
		GLfloat TransM[6] = { -CubicRota.x, -CubicRota.y, 0, -CubicPosi.x, -CubicPosi.y, -CubicPosi.z };
	...
	{	
		...
		/////////--------------------------------------------------------------------////////
		//for (int i = ROIStorageCount - 1; i > 0; --i)
		//{
		//	for (int j = 0; j < i; ++j)
		//	{
		//		if (ROICameraSP_Storage[j].Z > ROICameraSP_Storage[j + 1].Z)
		//		{
		//			CameraSpacePoint temp;

		//			temp.X = ROICameraSP_Storage[j].X;
		//			temp.Y = ROICameraSP_Storage[j].Y;
		//			temp.Z = ROICameraSP_Storage[j].Z;

		//			ROICameraSP_Storage[j].X = ROICameraSP_Storage[j + 1].X;
		//			ROICameraSP_Storage[j].Y = ROICameraSP_Storage[j + 1].Y;
		//			ROICameraSP_Storage[j].Z = ROICameraSP_Storage[j + 1].Z;

		//			ROICameraSP_Storage[j + 1].X = temp.X;
		//			ROICameraSP_Storage[j + 1].Y = temp.Y;
		//			ROICameraSP_Storage[j + 1].Z = temp.Z;
		...
		//for (int i = 0; i < ROIStorageCount; i++)
		//{
		//	cout << endl << ROICameraSP_Storage[i].Z;
		//}
		...
		//for (int i = 0; i < ROIStorageCount; i++)
		//{
		//	if (ROICameraSP_Storage[i].Z > ROICameraSP_Storage[ROIStorageCount-1].Z*1.5)
		//	{
		//		IndexLim = i;
		//		break;
		//	}
		...
		//Q1N = ROIStorageCount * 25.0f / 100.0f;
		//Q3N = ROIStorageCount * 75.0f / 100.0f;

		//float IQR = 0.0f;
		//IQR = ROICameraSP_Storage[Q3N].Z - ROICameraSP_Storage[Q1N].Z;

		//float UpLim = 0, DnLim = 0;
		//UpLim = ROICameraSP_Storage[Q1N].Z + 1.5*IQR;
		//DnLim = ROICameraSP_Storage[Q3N].Z - 1.5*IQR;

		//int UpIndex = 0, DnIndex = 0;

		//for (int i = 0; i < ROIStorageCount; i++)
		//{
		//	if (ROICameraSP_Storage[i].Z > DnLim)
		//	{
		//		DnIndex = i;
		//		break;
		//	}
		...
		//for (int i = ROIStorageCount - 1; i > -1 ; i--)
		//{
		//	if (ROICameraSP_Storage[i].Z < UpLim)
		//	{
		//		UpIndex = i;
		//		break;
		//	}
		...
		GLfloat TransM1[6] = { -CubicRota.x, -CubicRota.y, 0, -CubicPosi.x, -CubicPosi.y, -CubicPosi.z};
		GLfloat TransM2[6] = { CubicRota.x, CubicRota.y, 0, CubicPosi.x, CubicPosi.y, CubicPosi.z };
		...
		//GLfloat TransM1[6] = { 0, 0, 0, -CubicPosi.x, -CubicPosi.y, -CubicPosi.z };
		//GLfloat TransM2[6] = { CubicRota.x, CubicRota.y, 0, CubicPosi.x, CubicPosi.y, CubicPosi.z };
		//double Coeff[4] = { 0, 0, 1, -CubicUnitT };
		...
		for (int i = 0; i < ROIStorageCount; i++)
		{
			...
			//GLfloat pX = ROICameraSP_Projection[i].X;
			//GLfloat pY = ROICameraSP_Projection[i].Y;
			//GLfloat pZ = ROICameraSP_Projection[i].Z;
...
void KineticInit()
{
	...
	if (pSensor->get_CoordinateMapper(&Mapper) != S_OK)
	{
	 ...
	 CubicPosi.x = 0.069f - 0.1f;
	 CubicPosi.y = -0.0647f - 0.03f;
	 CubicPosi.z = -0.4227f - 0.5f;
...
void CameraSpaceROI()
{
	...
	//cout << endl << "outofrange: " << outofrange << endl;
...
void ROICameraSPStorage()
{
	...
	if (IS_KEY_F1_UP && ROIDepthCount > 0)
	{
		for (int i = ROIDepthCount - 1; i > 0; --i)
		{
			for (int j = 0; j < i; ++j)
			{
				if (ROICameraSP[j].Z < ROICameraSP[j + 1].Z)
				{
					CameraSpacePoint temp;

					temp.X = ROICameraSP[j].X;
					temp.Y = ROICameraSP[j].Y;
					temp.Z = ROICameraSP[j].Z;

					ROICameraSP[j].X = ROICameraSP[j + 1].X;
					ROICameraSP[j].Y = ROICameraSP[j + 1].Y;
					ROICameraSP[j].Z = ROICameraSP[j + 1].Z;

					ROICameraSP[j + 1].X = temp.X;
					ROICameraSP[j + 1].Y = temp.Y;
					ROICameraSP[j + 1].Z = temp.Z;
		...
		for (int i = 0; i < ROIDepthCount; i++)
		{
			if (ROICameraSP[i].Z < ROICameraSP[0].Z*1.02)
			{
				IndexLim = i;
				break;
			}
		...
		{
			if (ROIStorageCount + IndexLim < StorageSize)
			{
