...
//void PathGenCoordWorldToCubic(float* Data, int DataNum, float* Result);
...
int glPrintf(const char *format, ...)
{
	...
	//cout << endl << p0[0] << " " << p0[1] << " " << p0[2] << " " << p0[3] << endl;
	...
	if (ROIDepthCount != 0)
	{
		...
		glBegin(GL_POINTS);
			for (int i = 0; i < ROIDepthCount; i++)
			{
				...
				else if (pZ - CubicPosi.z > CubicUnitT)
				{
					ROI_IS_FRONT = TRUE;
				}
		...
		if (!ROI_IS_FRONT)
		{
			...
			glPrintf("(%.2f, %.2f, %.2f)", ROICenterCameraS.x, ROICenterCameraS.y, ROICenterCameraS.z);
		}
